...
// get things ready to transfer a file from freyja -> Mythic
...
// get things ready to transfer a file from Mythic -> freyja
...
// Tasks send a new net.Conn object to the task.Job.AddNewInternalConnectionChannel for freyja to track
...
func readFromInternalTCPConnections(newConnection net.Conn, tempConnectionUUID string) {
	...
	for {
		...
		if sizeBuffer > 0 {
			...
			newDelegateMessage.C2ProfileName = "freyja_tcp"
...
func handleRemoveInternalTCPConnections() {
	for {
		...
		case removeConnection := <-profiles.RemoveInternalTCPConnectionChannel:
			...
			removalMessage := structs.P2PConnectionMessage{Action: "remove", C2ProfileName: "freyja_tcp", Destination: removeConnection, Source: profiles.GetMythicID()}
